...
enum Plan[+E <: PlanError] {
  ...
  final def show(using Quotes): String = {
    ...
    this match {
      case Upcast(sourceTpe, destTpe) => s"Upcast(${sourceTpe.show}, ${destTpe.show})"
      case BetweenProducts(sourceTpe, destTpe, fieldPlans) => s"BetweenProducts(${sourceTpe.show}, ${destTpe.show}, ${fieldPlans.map((name, plan) => name -> plan.show)})"
      case BetweenCoproducts(sourceTpe, destTpe, casePlans) => s"BetweenCoproducts(${sourceTpe.show}, ${destTpe.show}, ${casePlans.map((name, plan) => name -> plan.show)})"
      case BetweenOptions(sourceTpe, destTpe, plan) => s"BetweenOptions(${sourceTpe.show}, ${destTpe.show}, ${plan.show})"
      case BetweenNonOptionOption(sourceTpe, destTpe, plan) => s"BetweenNonOptionOption(${sourceTpe.show}, ${destTpe.show}, ${plan.show})"
      case BetweenCollections(destCollectionTpe, sourceTpe, destTpe, plan) => s"BetweenCollections(${destCollectionTpe.show}, ${sourceTpe.show}, ${destTpe.show}, ${plan.show})"
      case BetweenSingletons(sourceTpe, destTpe, expr) => s"BetweenSingletons(${sourceTpe.show}, ${destTpe.show}, Expr(...))"
      case UserDefined(sourceTpe, destTpe, transformer) => s"UserDefined(${sourceTpe.show}, ${destTpe.show}, Transformer[...])"
      case PlanError(sourceTpe, destTpe, context, message) => s"Error(${sourceTpe.show}, ${destTpe.show}, ${context.render}, ${message})"
    }
  }

  final def updateAt[EE >: E <: Plan.Error](paths: List[String | Type[?]])(update: Plan[EE] => Plan[EE])(using Quotes): Option[Plan[EE]] = {
          ...
          case (tpe: Type[?]) :: next =>
            val result = PartialFunction.condOpt(plan) {
              case plan @ BetweenCoproducts(sourceTpe, destTpe, casePlans) =>
                for {
                  (name, ogCasePlan) <- casePlans.collectFirst { case (name, plan) if tpe.repr =:= plan.sourceTpe.repr => name -> plan }
