...
final class DefinitionBuilder[Source, Dest] {
  def accumulating[F[+x]](using Transformer.Accumulating.Support[F]): DefinitionBuilder.Accumulating[F, Source, Dest] =
    DefinitionBuilder.Accumulating[F, Source, Dest]

  def failFast[F[+x]](using Transformer.FailFast.Support[F]): DefinitionBuilder.FailFast[F, Source, Dest] =
    DefinitionBuilder.FailFast[F, Source, Dest]
...
object DefinitionBuilder {
  ...
  final class FailFast[F[+x], Source, Dest] private[ducktape] (using private val F: Transformer.FailFast.Support[F]) {
    ...
    )(using Mirror.ProductOf[Source], Mirror.ProductOf[Dest]): Transformer.FailFast[F, Source, Dest] =
      new {
  ...
  final class Accumulating[F[+x], Source, Dest] private[ducktape] (using private val F: Transformer.Accumulating.Support[F]) {
    inline def build(
      inline config: FallibleBuilderConfig[F, Source, Dest] | BuilderConfig[Source, Dest]*
    )(using Mirror.ProductOf[Source], Mirror.ProductOf[Dest]): Transformer.Accumulating[F, Source, Dest] =
      new {
