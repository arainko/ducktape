...
class RegionalFlagSuite extends DucktapeSuite {
  ...
  test("dest field regional flag covers all subtypes of an enum and everything below (even when the enum is nested, and we pick one of the sub-enums)") {
    case class Source(int: Int, level1: SourceEnum)
    case class Dest(int: Int, level1: DestEnum)
  ...
  test("source field regional flag covers all subtypes of an enum and everything below (even when the enum is nested, and we pick one of the sub-enums)") {
    case class Source(int: Int, level1: SourceEnum)
    case class Dest(int: Int, level1: DestEnum)
